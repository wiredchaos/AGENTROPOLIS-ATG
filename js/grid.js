...
async function bootThreeScene(){
  camPos = new THREE.Vector3(...KEY[0][0]);
  camLook= new THREE.Vector3(...KEY[0][1]);
  tPos   = camPos.clone();
  tLook  = camLook.clone();
...
async function loadRenderer(){
  try{
    ...
    await bootThreeScene();
  }catch(error){
    console.warn('[grid] 3D core unavailable — continuing with static experience.', error);
